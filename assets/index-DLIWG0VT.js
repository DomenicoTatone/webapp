(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function t(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(a){if(a.ep)return;a.ep=!0;const s=t(a);fetch(a.href,s)}})();class y{constructor(e,t={}){this.originalSelect=e,this.options={searchable:t.searchable||!1,placeholder:t.placeholder||"Select...",...t},this.isOpen=!1,this.selectedIndex=-1,this.focusedIndex=-1,this.id=`custom-select-${Math.random().toString(36).substr(2,9)}`,this.init()}init(){this.originalSelect.style.display="none",this.originalSelect.setAttribute("aria-hidden","true"),this.container=this.createContainer(),this.originalSelect.parentNode.insertBefore(this.container,this.originalSelect.nextSibling),this.syncFromOriginal(),this.bindEvents()}createContainer(){const e=document.createElement("div");return e.className="custom-select",e.innerHTML=`
        ...
        `,e}renderOptions(){const e=this.originalSelect.querySelectorAll("option");return Array.from(e).map((t,n)=>{const a=t.selected,s=t.disabled;return`
                <li class="custom-select__option ${a?"is-selected":""} ${s?"is-disabled":""}"
                    role="option"
                    aria-selected="${a}"
                    data-value="${t.value}"
                    data-index="${n}"
    ...
    `,document.body.appendChild(this.container),!document.getElementById("toast-styles"))){const e=document.createElement("style");e.id="toast-styles",e.textContent=`
      ...
      cursor: pointer;
    `,a.innerHTML=`<span style="font-size: 18px;">${s[t]}</span><span>${e}</span>`,a.addEventListener("click",()=>this.dismiss(a)),this.container.appendChild(a),n>0&&setTimeout(()=>this.dismiss(a),n),a}dismiss(e){e.style.animation="slideOut 0.3s ease forwards",setTimeout(()=>e.remove(),300)}success(e,t){return this.show(e,"success",t)}error(e,t){return this.show(e,"error",t)}warning(e,t){return this.show(e,"warning",t)}info(e,t){return this.show(e,"info",t)}}const p=new A,_="https://allspainbookinglinks.s3.eu-west-3.amazonaws.com",E={hotelPage:{file:l=>`hotel_page_links_${l}.js`,variableName:l=>`links${l.toUpperCase()}`,hasSubType:!1},cityPage:{file:(l,e)=>e==="landing"?`city_landing_page_links_${l}.js`:`city_page_links_${l}.js`,variableName:(l,e)=>e==="landing"?`linksLandingCityPage${l.toUpperCase()}`:`linksCityPage${l.toUpperCase()}`,hasSubType:!0},airportPage:{file:(l,e)=>e==="landing"?`airport_landing_page_links_${l}.js`:`airport_page_links_${l}.js`,variableName:(l,e)=>e==="landing"?`linksAirportLanding${l.toUpperCase()}`:`linksAirport${l.toUpperCase()}`,hasSubType:!0},districtPage:{file:(l,e)=>e==="landing"?`district_landing_page_links_${l}.js`:`district_page_links_${l}.js`,variableName:(l,e)=>e==="landing"?`linksDistrictLandingPage${l.toUpperCase()}`:`linksDistrictPage${l.toUpperCase()}`,hasSubType:!0},islandPage:{file:(l,e)=>e==="landing"?`island_landing_page_links_${l}.js`:`island_page_links_${l}.js`,variableName:(l,e)=>e==="landing"?`linksLandingIslandPage${l.toUpperCase()}`:`linksIslandPage${l.toUpperCase()}`,hasSubType:!0},landmarkPage:{file:(l,e)=>e==="landing"?`landmark_landing_page_links_${l}.js`:`landmark_page_links_${l}.js`,variableName:(l,e)=>e==="landing"?`linksLandmarkLandingPage${l.toUpperCase()}`:`linksLandmarkPage${l.toUpperCase()}`,hasSubType:!0},regionPage:{file:(l,e)=>e==="landing"?`region_landing_page_links_${l}.js`:`region_page_links_${l}.js`,variableName:(l,e)=>e==="landing"?`linksRegionLandingPage${l.toUpperCase()}`:`linksRegionPage${l.toUpperCase()}`,hasSubType:!0}};class x{constructor(){this.dataCache=new Map,this.loadingPromises=new Map,this.STORAGE_PREFIX="bookingData_",this.CACHE_EXPIRY_MS=1440*60*1e3}getCacheKey(e,t,n=null){return`${e}_${t}_${n||"default"}`}getFromStorage(e){try{const t=localStorage.getItem(this.STORAGE_PREFIX+e);if(!t)return null;const{data:n,timestamp:a}=JSON.parse(t),s=Date.now()-a;return s>this.CACHE_EXPIRY_MS?(localStorage.removeItem(this.STORAGE_PREFIX+e),console.log(`[BookingData] Storage expired for ${e}`),null):(console.log(`[BookingData] Storage hit for ${e} (age: ${Math.round(s/6e4)}min)`),n)}catch(t){return console.warn("[BookingData] Storage read error:",t),null}}saveToStorage(e,t){try{const n=JSON.stringify({data:t,timestamp:Date.now()});localStorage.setItem(this.STORAGE_PREFIX+e,n),console.log(`[BookingData] Saved to storage: ${e}`)}catch(n){console.warn("[BookingData] Storage write error:",n)}}async loadData(e,t,n=null){const a=this.getCacheKey(e,t,n);if(this.dataCache.has(a))return console.log(`[BookingData] Memory cache hit for ${a}`),this.dataCache.get(a);const s=this.getFromStorage(a);if(s)return this.dataCache.set(a,s),s;if(this.loadingPromises.has(a))return console.log(`[BookingData] Waiting for existing load: ${a}`),this.loadingPromises.get(a);const i=this._fetchData(e,t,n);this.loadingPromises.set(a,i);try{const o=await i;return this.dataCache.set(a,o),this.saveToStorage(a,o),console.log(`[BookingData] Loaded ${o.length} items for ${a}`),o}finally{this.loadingPromises.delete(a)}}async _fetchData(e,t,n){const a=E[e];if(!a)throw new Error(`Unknown page type: ${e}`);const s=a.file(t,n),i=a.variableName(t,n),o=`${_}/${s}`;return console.log(`[BookingData] Loading script: ${o}`),console.log(`[BookingData] Expected variable: ${i}`),window[i]&&Array.isArray(window[i])?(console.log("[BookingData] Variable already exists, using cached data"),window[i]):document.querySelector(`script[src="${o}"]`)&&(await this._waitForVariable(i,2e3),window[i])?window[i]:new Promise((r,u)=>{const g=document.createElement("script");g.src=o,g.async=!0,g.className="booking-data-script",g.onload=async()=>{console.log(`[BookingData] Script loaded: ${s}`);try{await this._waitForVariable(i,3e3),window[i]&&Array.isArray(window[i])?(console.log(`[BookingData] Found ${window[i].length} items`),r(window[i])):u(new Error(`Variable ${i} not found after script load`))}catch(h){u(h)}},g.onerror=()=>{console.error(`[BookingData] Failed to load script: ${o}`),u(new Error(`Failed to load ${s}`))},document.body.appendChild(g)})}_waitForVariable(e,t=3e3){return new Promise((n,a)=>{const s=Date.now(),i=()=>{window[e]!==void 0?n():Date.now()-s>t?a(new Error(`Timeout waiting for ${e}`)):setTimeout(i,50)};i()})}normalizeText(e){return e.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase()}search(e,t,n=50){if(!e||e.length<3)return[];const a=this.normalizeText(e),s=[];for(const i of t){if(s.length>=n)break;this.normalizeText(i[0]||"").includes(a)&&s.push(i)}return s}formatResult(e,t){const n=e[e.length-1];switch(t){case"hotelPage":return{name:e[0],subtitle:e[1]||"",url:n};case"airportPage":return{name:e[0],subtitle:e[1]||"",url:n};case"districtPage":return{name:e[0],subtitle:e[1]||"",url:n};default:return{name:e[0],subtitle:e.length>2?e[1]:"",url:n}}}hasSubType(e){return E[e]?.hasSubType||!1}clearCache(){this.dataCache.clear(),console.log("[BookingData] Cache cleared")}}const b=new x;class ${constructor(){this.currentPage="booking",this.bookingState={data:null,isLoading:!1,error:null,searchTimeout:null}}async init(){await c.init(),console.log(`[App] Language detected: ${c.getLanguage()}`),this.setupNavigation(),this.setupLanguageSelector(),this.loadPage(this.currentPage)}setupNavigation(){document.querySelectorAll("[data-page]").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault(),this.loadPage(t.currentTarget.dataset.page)})})}setupLanguageSelector(){const e=document.querySelector(".language-dropdown"),t=document.getElementById("langTrigger");document.getElementById("langMenu");const n=document.querySelectorAll(".lang-option"),a={it:{flag:"🇮🇹",code:"IT"},en:{flag:"🇬🇧",code:"EN"},es:{flag:"🇪🇸",code:"ES"},fr:{flag:"🇫🇷",code:"FR"},de:{flag:"🇩🇪",code:"DE"}},s=c.getLanguage();this.updateLangTrigger(t,a[s]),document.querySelector(`[data-lang="${s}"]`)?.classList.add("active"),t?.addEventListener("click",i=>{i.stopPropagation(),e.classList.toggle("is-open"),t.setAttribute("aria-expanded",e.classList.contains("is-open"))}),document.addEventListener("click",i=>{e?.contains(i.target)||(e?.classList.remove("is-open"),t?.setAttribute("aria-expanded","false"))}),n.forEach(i=>{i.addEventListener("click",async()=>{const o=i.dataset.lang;n.forEach(d=>d.classList.remove("active")),i.classList.add("active"),this.updateLangTrigger(t,a[o]),e.classList.remove("is-open"),t.setAttribute("aria-expanded","false"),await c.setLanguage(o),b.clearCache(),this.bookingState.data=null,this.loadPage(this.currentPage),p.success(c.t("languageChanged"))})})}updateLangTrigger(e,t){!e||!t||(e.querySelector(".lang-flag").textContent=t.flag,e.querySelector(".lang-code").textContent=t.code)}loadPage(e){this.currentPage=e;const t=document.getElementById("page-content");document.querySelectorAll("[data-page]").forEach(a=>{a.classList.toggle("active",a.dataset.page===e)}),({booking:()=>{t.innerHTML=this.renderBookingPage(),this.initBookingPage()},tradedoubler:()=>{t.innerHTML=this.renderTradedoublerPage(),this.initTradedoublerPage()},getyourguide:()=>{t.innerHTML=this.renderGetYourGuidePage(),this.initGetYourGuidePage()},civitatis:()=>{t.innerHTML=this.renderCivitatisPage(),this.initCivitatisPage()},carrental:()=>{t.innerHTML=this.renderCarRentalPage(),this.initCarRentalPage()},imgtool:()=>{t.innerHTML=this.renderImageToolPage(),this.initImageToolPage()},feedback:()=>{t.innerHTML=this.renderFeedbackPage(),this.initFeedbackPage()}}[e]||(()=>{t.innerHTML="<p>Page not found</p>"}))(),c.translatePage()}renderBookingPage(){return`
    ...
    `}async initBookingPage(){const e=document.getElementById("pageTypeSelect"),t=document.getElementById("subTypeGroup"),n=document.getElementById("searchInput"),a=document.getElementById("searchHint"),s=document.getElementById("landingLinksCard");e&&new y(e);const i=document.getElementById("bookingLangSelect");i&&new y(i);const o=async()=>{const h=e.value,f=h==="genericLandingPages",S=b.hasSubType(h);t.style.display=S?"block":"none",n.style.display=f?"none":"block",a&&(a.style.display=f?"none":"block"),s.style.display=f?"block":"none",document.getElementById("resultsHeader").style.display="none",document.getElementById("resultsContainer").innerHTML="",document.getElementById("errorState").style.display="none",f?this.showGenericLandingPages():await this.loadBookingData()},d=()=>{clearTimeout(this.bookingState.searchTimeout),this.bookingState.searchTimeout=setTimeout(()=>{this.performSearch(n.value)},300)},r=async()=>{this.bookingState.data=null,await this.loadBookingData()},u=document.getElementById("bookingLangSelect"),g=async()=>{this.bookingState.data=null,await this.loadBookingData()};e.addEventListener("change",o),n.addEventListener("input",d),u.addEventListener("change",g),document.querySelectorAll('input[name="subType"]').forEach(h=>{h.addEventListener("change",r)}),document.getElementById("retryBtn")?.addEventListener("click",()=>this.loadBookingData()),await o()}async loadBookingData(){const e=document.getElementById("pageTypeSelect").value;if(e==="genericLandingPages")return;const t=document.getElementById("bookingLangSelect")?.value||"it",a=document.querySelector('input[name="subType"]:checked')?.value==="landing"?"landing":null;this.setBookingUIState("loading");try{const s=await b.loadData(e,t,a);this.bookingState.data=s,this.bookingState.error=null,this.setBookingUIState("ready"),p.success(c.t("dataLoaded"));const i=document.getElementById("searchInput")?.value;i&&i.length>=3&&this.performSearch(i)}catch(s){console.error("[Booking] Load error:",s),this.bookingState.error=s.message,this.setBookingUIState("error")}}setBookingUIState(e){const t=document.getElementById("loadingState"),n=document.getElementById("errorState"),a=document.getElementById("resultsHeader"),s=document.getElementById("resultsContainer"),i=document.getElementById("searchHint");t.style.display=e==="loading"?"flex":"none",n.style.display=e==="error"?"flex":"none",a.style.display="none",s.innerHTML="",i&&(i.style.display=e==="ready"?"block":"none")}performSearch(e){if(!this.bookingState.data)return;const t=document.getElementById("resultsHeader"),n=document.getElementById("resultsContainer"),a=document.getElementById("resultsCount"),s=document.getElementById("searchHint");if(!e||e.length<3){t.style.display="none",n.innerHTML="",s&&(s.style.display="block");return}s&&(s.style.display="none");const i=document.getElementById("pageTypeSelect").value,o=b.search(e,this.bookingState.data,50);if(o.length===0){t.style.display="flex",a.textContent=c.t("noResultsFound"),n.innerHTML=`
        <div class="empty-state">
          <span class="empty-icon">🔍</span>
          <p data-i18n="noResultsFound">${c.t("noResultsFound")}</p>
        </div>
      `;return}t.style.display="flex",a.textContent=`${o.length} ${c.t("resultsFound")}`,n.innerHTML=o.map(d=>{const r=b.formatResult(d,i);return`
        <div class="result-card">
          ...
          <div class="result-actions">
            <button class="btn btn-sm btn-primary" data-copy="${this.escapeHtml(r.url)}">${c.t("copyLink")}</button>
            <a href="${r.url}" target="_blank" class="btn btn-sm btn-outline">${c.t("openLink")}</a>
          </div>
        </div>
      `}).join(""),n.querySelectorAll("[data-copy]").forEach(d=>{d.addEventListener("click",async()=>{const r=await m.copyToClipboard(d.dataset.copy);p[r?"success":"error"](c.t(r?"deepLinkCopied":"copyError"))})})}escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}showGenericLandingPages(){const e=document.getElementById("landingLinks");document.getElementById("landingLinksCard").style.display="block",document.getElementById("resultsCard").style.display="none";const t=[{key:"homePage",url:"https://www.booking.com/index.html?aid=955564"},{key:"apartmentsPage",url:"https://www.booking.com/apartments/index.html?aid=955564"},{key:"resortsPage",url:"https://www.booking.com/resorts/index.html?aid=955564"},{key:"villasPage",url:"https://www.booking.com/villas/index.html?aid=955564"},{key:"bedAndBreakfastPage",url:"https://www.booking.com/bed-and-breakfast/index.html?aid=955564"},{key:"guestHousePage",url:"https://www.booking.com/guest-house/index.html?aid=955564"}];e.innerHTML=t.map(n=>`
      <div class="link-item">
        <span>${c.t(n.key)}</span>
        <div class="link-actions">
          <a href="${n.url}" target="_blank" class="btn btn-sm btn-outline">${c.t("openLink")}</a>
          <button class="btn btn-sm btn-primary" data-copy="${n.url}">${c.t("copyLink")}</button>
        </div>
      </div>
    `).join(""),e.querySelectorAll("[data-copy]").forEach(n=>{n.addEventListener("click",async()=>{const a=await m.copyToClipboard(n.dataset.copy);p[a?"success":"error"](c.t(a?"deepLinkCopied":"copyError"))})})}renderTradedoublerPage(){return`
      <div class="page-header">
    ...
    `}initTradedoublerPage(){const e=document.getElementById("islandSelect"),t=document.getElementById("platformSelect"),n=document.getElementById("partnerLinkContainer"),a=document.getElementById("partnerLink"),s=document.getElementById("generateBtn"),i=document.getElementById("inputUrl"),o=document.getElementById("resultContainer"),d=document.getElementById("resultLink");e&&new y(e);let r=t?new y(t):null;e.addEventListener("change",()=>{const u=e.value;t.innerHTML=`<option value="">${c.t("selectPlatform")}</option>`,u?((B[u]||[]).forEach(g=>{t.appendChild(new Option(g,g))}),t.disabled=!1):t.disabled=!0,r&&r.destroy(),r=new y(t),n.style.display="none",o.style.display="none"}),t.addEventListener("change",()=>{const u=e.value,g=t.value;if(u&&g){const h=m.getPartnerBaseLink(u,g);h&&(a.innerHTML=`<a href="${h}" target="_blank">${h}</a>`,n.style.display="block")}else n.style.display="none"}),s.addEventListener("click",()=>{const u=e.value,g=t.value,h=i.value.trim();if(!u||!g){p.warning(c.t("selectSiteAndPlatform"));return}if(!h){p.warning(c.t("enterUrl"));return}const f=m.generateDeepLink(h,u,g);f.success?(d.href=f.link,d.textContent=f.link,o.style.display="block",p.success(c.t("deepLinkGenerated"))):p.error(c.t(f.error)||f.error)}),document.getElementById("copyResultBtn")?.addEventListener("click",async()=>{const u=await m.copyToClipboard(d.href);p[u?"success":"error"](c.t(u?"deepLinkCopied":"copyError"))}),document.getElementById("openResultBtn")?.addEventListener("click",()=>{m.openLink(d.href)})}renderGetYourGuidePage(){return`
      <div class="page-header">
    ...
    `}initGetYourGuidePage(){const e={it:"https://www.getyourguide.it/?partner_id=Q5TFESQ&utm_medium=online_publisher",es:"https://www.getyourguide.es/?partner_id=Q5TFESQ&utm_medium=online_publisher",en:"https://www.getyourguide.com/?partner_id=Q5TFESQ&utm_medium=online_publisher",fr:"https://www.getyourguide.fr/?partner_id=Q5TFESQ&utm_medium=online_publisher"},t=document.getElementById("gygLangSelect"),n=document.getElementById("gygHomepageLink");t&&new y(t);const a=()=>{const r=e[t.value]||e.en;n.href=r,n.textContent=r};t.addEventListener("change",a),a(),document.getElementById("gygHomepageCopyBtn")?.addEventListener("click",async()=>{const r=await m.copyToClipboard(n.href);p[r?"success":"error"](c.t(r?"deepLinkCopied":"copyError"))}),document.getElementById("gygHomepageOpenBtn")?.addEventListener("click",()=>{m.openLink(n.href)});const s=document.getElementById("gygInput"),i=document.getElementById("gygGenerateBtn"),o=document.getElementById("gygResult"),d=document.getElementById("gygResultLink");i.addEventListener("click",()=>{const r=s.value.trim();if(!r){p.warning(c.t("validGetYourGuideURL"));return}const u=m.generateGetYourGuideLink(r);u.success?(d.href=u.link,d.textContent=u.link,o.style.display="block",p.success(c.t("deepLinkGenerated"))):p.error(c.t(u.error))}),document.getElementById("gygCopyBtn")?.addEventListener("click",async()=>{const r=await m.copyToClipboard(d.href);p[r?"success":"error"](c.t(r?"deepLinkCopied":"copyError"))}),document.getElementById("gygOpenBtn")?.addEventListener("click",()=>{m.openLink(d.href)})}renderCivitatisPage(){return`
      <div class="page-header">
    ...
    `}initCivitatisPage(){const e={it:"https://www.civitatis.com/it/?aid=5488",es:"https://www.civitatis.com/es/?aid=5488",en:"https://www.civitatis.com/en/?aid=5488",fr:"https://www.civitatis.com/fr/?aid=5488"},t=document.getElementById("civLangSelect"),n=document.getElementById("civHomepageLink");t&&new y(t);const a=()=>{const r=e[t.value]||e.en;n.href=r,n.textContent=r};t.addEventListener("change",a),a(),document.getElementById("civHomepageCopyBtn")?.addEventListener("click",async()=>{const r=await m.copyToClipboard(n.href);p[r?"success":"error"](c.t(r?"deepLinkCopied":"copyError"))}),document.getElementById("civHomepageOpenBtn")?.addEventListener("click",()=>{m.openLink(n.href)});const s=document.getElementById("civInput"),i=document.getElementById("civGenerateBtn"),o=document.getElementById("civResult"),d=document.getElementById("civResultLink");i.addEventListener("click",()=>{const r=s.value.trim();if(!r){p.warning(c.t("validCivitatisURL"));return}const u=m.generateCivitatisLink(r);u.success?(d.href=u.link,d.textContent=u.link,o.style.display="block",p.success(c.t("deepLinkGenerated"))):p.error(c.t(u.error))}),document.getElementById("civCopyBtn")?.addEventListener("click",async()=>{const r=await m.copyToClipboard(d.href);p[r?"success":"error"](c.t(r?"deepLinkCopied":"copyError"))}),document.getElementById("civOpenBtn")?.addEventListener("click",()=>{m.openLink(d.href)})}renderCarRentalPage(){return`
      <div class="page-header">
    ...
    `}initCarRentalPage(){const e=document.getElementById("carProviderSelect"),t=document.getElementById("carLangSelect"),n=document.getElementById("carResultLink");e&&new y(e),t&&new y(t);const a=()=>{const s=I[e.value],i=t.value,o=s.urls[i]||s.urls.en||Object.values(s.urls)[0];n.href=o,n.textContent=o};e.addEventListener("change",a),t.addEventListener("change",a),a(),document.getElementById("carCopyBtn")?.addEventListener("click",async()=>{const s=await m.copyToClipboard(n.href);p[s?"success":"error"](c.t(s?"deepLinkCopied":"copyError"))}),document.getElementById("carOpenBtn")?.addEventListener("click",()=>{m.openLink(n.href)})}renderImageToolPage(){return`
      <div class="page-header">
    ...
    `}initImageToolPage(){const e=document.getElementById("dropZone"),t=document.getElementById("imageInput"),n=document.getElementById("qualitySlider"),a=document.getElementById("qualityValue"),s=document.getElementById("formatSelect"),i=document.getElementById("imagesContainer"),o=document.getElementById("downloadAllContainer");s&&new y(s),this.compressedImages=[],n.addEventListener("input",()=>{a.textContent=`${n.value}%`}),e.addEventListener("click",()=>t.click()),["dragenter","dragover","dragleave","drop"].forEach(d=>{e.addEventListener(d,r=>{r.preventDefault(),r.stopPropagation()})}),e.addEventListener("dragenter",()=>e.classList.add("drag-over")),e.addEventListener("dragover",()=>e.classList.add("drag-over")),e.addEventListener("dragleave",()=>e.classList.remove("drag-over")),e.addEventListener("drop",d=>{e.classList.remove("drag-over");const r=d.dataTransfer.files;this.processImages(r)}),t.addEventListener("change",()=>{this.processImages(t.files)}),document.getElementById("clearAllBtn")?.addEventListener("click",()=>{this.compressedImages=[],i.innerHTML="",o.style.display="none"}),document.getElementById("downloadAllBtn")?.addEventListener("click",async()=>{this.compressedImages.length!==0&&await this.downloadAllAsZip()})}async processImages(e){const t=document.getElementById("qualitySlider").value/100,n=document.getElementById("formatSelect").value,a=document.getElementById("imagesContainer"),s=document.getElementById("downloadAllContainer");for(const i of e)if(i.type.startsWith("image/"))try{const o=await this.compressImage(i,t,n);this.compressedImages.push(o),this.renderImageCard(o,a),this.updateTotalStats(),s.style.display="block"}catch(o){console.error("Compression error:",o),p.error(`Errore: ${i.name}`)}}compressImage(e,t,n){return new Promise((a,s)=>{const i=new FileReader;i.onload=o=>{const d=new Image;d.onload=()=>{const r=document.createElement("canvas");r.width=d.naturalWidth,r.height=d.naturalHeight,r.getContext("2d").drawImage(d,0,0),r.toBlob(g=>{if(!g){s(new Error("Compression failed"));return}const h=n==="image/webp"?"webp":n==="image/png"?"png":"jpg",f=e.name.replace(/\.[^/.]+$/,"");a({originalName:e.name,compressedName:`${f}_compressed.${h}`,originalSize:e.size,compressedSize:g.size,reduction:((1-g.size/e.size)*100).toFixed(1),blob:g,url:URL.createObjectURL(g)})},n,t)},d.onerror=s,d.src=o.target.result},i.onerror=s,i.readAsDataURL(e)})}renderImageCard(e,t){const n=document.createElement("div");n.className="image-card";const a=i=>i>1024*1024?`${(i/1024/1024).toFixed(2)} MB`:`${(i/1024).toFixed(1)} KB`,s=parseFloat(e.reduction)>50?"text-success":parseFloat(e.reduction)>20?"text-warning":"text-muted";n.innerHTML=`
    ...
    `,n.querySelector(".download-btn").addEventListener("click",()=>{const i=document.createElement("a");i.href=e.url,i.download=e.compressedName,i.click()}),n.querySelector(".remove-btn").addEventListener("click",()=>{const i=this.compressedImages.findIndex(o=>o.url===e.url);i>-1&&(URL.revokeObjectURL(e.url),this.compressedImages.splice(i,1),n.remove(),this.updateTotalStats(),this.compressedImages.length===0&&(document.getElementById("downloadAllContainer").style.display="none"))}),t.appendChild(n)}updateTotalStats(){const e=document.getElementById("totalStats");if(!e||this.compressedImages.length===0)return;const t=this.compressedImages.reduce((i,o)=>i+o.originalSize,0),n=this.compressedImages.reduce((i,o)=>i+o.compressedSize,0),a=((1-n/t)*100).toFixed(1),s=i=>i>1024*1024?`${(i/1024/1024).toFixed(2)} MB`:`${(i/1024).toFixed(1)} KB`;e.textContent=`${this.compressedImages.length} ${c.t("images")} • ${s(t)} → ${s(n)} (-${a}%)`}async downloadAllAsZip(){if(!window.JSZip){const a=document.createElement("script");a.src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js",document.head.appendChild(a),await new Promise(s=>a.onload=s)}const e=new JSZip;for(const a of this.compressedImages)e.file(a.compressedName,a.blob);const t=await e.generateAsync({type:"blob"}),n=document.createElement("a");n.href=URL.createObjectURL(t),n.download="compressed_images.zip",n.click(),URL.revokeObjectURL(n.href),p.success(c.t("downloadStarted"))}renderFeedbackPage(){return`
      <div class="page-header">
