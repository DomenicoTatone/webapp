(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function t(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(a){if(a.ep)return;a.ep=!0;const s=t(a);fetch(a.href,s)}})();class b{constructor(e,t={}){this.originalSelect=e,this.options={searchable:t.searchable||!1,placeholder:t.placeholder||"Select...",...t},this.isOpen=!1,this.selectedIndex=-1,this.focusedIndex=-1,this.id=`custom-select-${Math.random().toString(36).substr(2,9)}`,this.init()}init(){this.originalSelect.style.display="none",this.originalSelect.setAttribute("aria-hidden","true"),this.container=this.createContainer(),this.originalSelect.parentNode.insertBefore(this.container,this.originalSelect.nextSibling),this.syncFromOriginal(),this.bindEvents()}createContainer(){const e=document.createElement("div");return e.className="custom-select",e.innerHTML=`
    ...
    `).join(""),e.querySelectorAll("[data-copy]").forEach(n=>{n.addEventListener("click",async()=>{const a=await g.copyToClipboard(n.dataset.copy);p[a?"success":"error"](c.t(a?"deepLinkCopied":"copyError"))})})}renderTradedoublerPage(){return`
      <div class="page-header">
    ...
    `}initTradedoublerPage(){const e=document.getElementById("islandSelect"),t=document.getElementById("platformSelect"),n=document.getElementById("partnerLinkContainer"),a=document.getElementById("partnerLink"),s=document.getElementById("generateBtn"),i=document.getElementById("inputUrl"),r=document.getElementById("resultContainer"),d=document.getElementById("resultLink"),l=document.getElementById("programCardsContainer");e&&new b(e);let m=t?new b(t):null;const y=u=>{const h=I[u]||[];if(!u||h.length===0){l.innerHTML=`
          <div class="program-cards-empty">
            <span class="empty-icon">📋</span>
            <p data-i18n="selectSiteToViewPrograms">Seleziona un sito per vedere i programmi attivi</p>
          </div>
        `;return}const f=h.map(v=>{const k=g.getPartnerBaseLink(u,v);return`
    ...
    `}initCarRentalPage(){const e=document.getElementById("carProviderSelect"),t=document.getElementById("carLangSelect"),n=document.getElementById("carResultLink");e&&new b(e),t&&new b(t);const a=()=>{const s=B[e.value],i=t.value,r=s.urls[i]||s.urls.en||Object.values(s.urls)[0];n.href=r,n.textContent=r};e.addEventListener("change",a),t.addEventListener("change",a),a(),document.getElementById("carCopyBtn")?.addEventListener("click",async()=>{const s=await g.copyToClipboard(n.href);p[s?"success":"error"](c.t(s?"deepLinkCopied":"copyError"))}),document.getElementById("carOpenBtn")?.addEventListener("click",()=>{g.openLink(n.href)})}renderImageToolPage(){return`
      <div class="page-header">
    ...
    `}initImageToolPage(){const e=document.getElementById("dropZone"),t=document.getElementById("imageInput"),n=document.getElementById("qualitySlider"),a=document.getElementById("qualityValue"),s=document.getElementById("formatSelect"),i=document.getElementById("imagesContainer"),r=document.getElementById("downloadAllContainer");s&&new b(s),this.compressedImages=[],n.addEventListener("input",()=>{a.textContent=`${n.value}%`}),e.addEventListener("click",()=>t.click()),["dragenter","dragover","dragleave","drop"].forEach(d=>{e.addEventListener(d,l=>{l.preventDefault(),l.stopPropagation()})}),e.addEventListener("dragenter",()=>e.classList.add("drag-over")),e.addEventListener("dragover",()=>e.classList.add("drag-over")),e.addEventListener("dragleave",()=>e.classList.remove("drag-over")),e.addEventListener("drop",d=>{e.classList.remove("drag-over");const l=d.dataTransfer.files;this.processImages(l)}),t.addEventListener("change",()=>{this.processImages(t.files)}),document.getElementById("clearAllBtn")?.addEventListener("click",()=>{this.compressedImages=[],i.innerHTML="",r.style.display="none"}),document.getElementById("downloadAllBtn")?.addEventListener("click",async()=>{this.compressedImages.length!==0&&await this.downloadAllAsZip()})}async processImages(e){const t=document.getElementById("qualitySlider").value/100,n=document.getElementById("formatSelect").value,a=document.getElementById("imagesContainer"),s=document.getElementById("downloadAllContainer");for(const i of e)if(i.type.startsWith("image/"))try{const r=await this.compressImage(i,t,n);this.compressedImages.push(r),this.renderImageCard(r,a),this.updateTotalStats(),s.style.display="block"}catch(r){console.error("Compression error:",r),p.error(`Errore: ${i.name}`)}}compressImage(e,t,n){return new Promise((a,s)=>{const i=new FileReader;i.onload=r=>{const d=new Image;d.onload=()=>{const l=document.createElement("canvas");l.width=d.naturalWidth,l.height=d.naturalHeight,l.getContext("2d").drawImage(d,0,0),l.toBlob(y=>{if(!y){s(new Error("Compression failed"));return}const u=n==="image/webp"?"webp":n==="image/png"?"png":"jpg",h=e.name.replace(/\.[^/.]+$/,"");a({originalName:e.name,compressedName:`${h}_compressed.${u}`,originalSize:e.size,compressedSize:y.size,reduction:((1-y.size/e.size)*100).toFixed(1),blob:y,url:URL.createObjectURL(y)})},n,t)},d.onerror=s,d.src=r.target.result},i.onerror=s,i.readAsDataURL(e)})}renderImageCard(e,t){const n=document.createElement("div");n.className="image-card";const a=i=>i>1024*1024?`${(i/1024/1024).toFixed(2)} MB`:`${(i/1024).toFixed(1)} KB`,s=parseFloat(e.reduction)>50?"text-success":parseFloat(e.reduction)>20?"text-warning":"text-muted";n.innerHTML=`
