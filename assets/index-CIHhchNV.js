(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const s of i.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&a(s)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();class f{constructor(e,t={}){this.originalSelect=e,this.options={searchable:t.searchable||!1,placeholder:t.placeholder||"Select...",...t},this.isOpen=!1,this.selectedIndex=-1,this.focusedIndex=-1,this.id=`custom-select-${Math.random().toString(36).substr(2,9)}`,this.init()}init(){this.originalSelect.style.display="none",this.originalSelect.setAttribute("aria-hidden","true"),this.container=this.createContainer(),this.originalSelect.parentNode.insertBefore(this.container,this.originalSelect.nextSibling),this.syncFromOriginal(),this.bindEvents()}createContainer(){const e=document.createElement("div");return e.className="custom-select",e.innerHTML=`
    ...
    `,document.body.appendChild(this.container),!document.getElementById("toast-styles"))){const e=document.createElement("style");e.id="toast-styles",e.textContent=`
      ...
      `,document.head.appendChild(e)}}show(e,t="info",a=3e3){this.init();const n=document.createElement("div"),i={success:"✓",error:"✕",warning:"⚠",info:"ℹ"},s={success:{bg:"#d4edda",border:"#28a745",text:"#155724"},error:{bg:"#f8d7da",border:"#dc3545",text:"#721c24"},warning:{bg:"#fff3cd",border:"#ffc107",text:"#856404"},info:{bg:"#d1ecf1",border:"#17a2b8",text:"#0c5460"}},o=s[t]||s.info;return n.style.cssText=`
      ...
      cursor: pointer;
    `,n.innerHTML=`<span style="font-size: 18px;">${i[t]}</span><span>${e}</span>`,n.addEventListener("click",()=>this.dismiss(n)),this.container.appendChild(n),a>0&&setTimeout(()=>this.dismiss(n),a),n}dismiss(e){e.style.animation="slideOut 0.3s ease forwards",setTimeout(()=>e.remove(),300)}success(e,t){return this.show(e,"success",t)}error(e,t){return this.show(e,"error",t)}warning(e,t){return this.show(e,"warning",t)}info(e,t){return this.show(e,"info",t)}}const p=new D,G="https://allspainbookinglinks.s3.eu-west-3.amazonaws.com",B={hotelPage:{file:r=>`hotel_page_links_${r}.js`,variableName:r=>`links${r.toUpperCase()}`,hasSubType:!1},cityPage:{file:(r,e)=>e==="landing"?`city_landing_page_links_${r}.js`:`city_page_links_${r}.js`,variableName:(r,e)=>e==="landing"?`linksLandingCityPage${r.toUpperCase()}`:`linksCityPage${r.toUpperCase()}`,hasSubType:!0},airportPage:{file:(r,e)=>e==="landing"?`airport_landing_page_links_${r}.js`:`airport_page_links_${r}.js`,variableName:(r,e)=>e==="landing"?`linksAirportLanding${r.toUpperCase()}`:`linksAirport${r.toUpperCase()}`,hasSubType:!0},districtPage:{file:(r,e)=>e==="landing"?`district_landing_page_links_${r}.js`:`district_page_links_${r}.js`,variableName:(r,e)=>e==="landing"?`linksDistrictLandingPage${r.toUpperCase()}`:`linksDistrictPage${r.toUpperCase()}`,hasSubType:!0},islandPage:{file:(r,e)=>e==="landing"?`island_landing_page_links_${r}.js`:`island_page_links_${r}.js`,variableName:(r,e)=>e==="landing"?`linksLandingIslandPage${r.toUpperCase()}`:`linksIslandPage${r.toUpperCase()}`,hasSubType:!0},landmarkPage:{file:(r,e)=>e==="landing"?`landmark_landing_page_links_${r}.js`:`landmark_page_links_${r}.js`,variableName:(r,e)=>e==="landing"?`linksLandmarkLandingPage${r.toUpperCase()}`:`linksLandmarkPage${r.toUpperCase()}`,hasSubType:!0},regionPage:{file:(r,e)=>e==="landing"?`region_landing_page_links_${r}.js`:`region_page_links_${r}.js`,variableName:(r,e)=>e==="landing"?`linksRegionLandingPage${r.toUpperCase()}`:`linksRegionPage${r.toUpperCase()}`,hasSubType:!0}};class O{constructor(){this.dataCache=new Map,this.loadingPromises=new Map,this.STORAGE_PREFIX="bookingData_",this.CACHE_EXPIRY_MS=1440*60*1e3}getCacheKey(e,t,a=null){return`${e}_${t}_${a||"default"}`}getFromStorage(e){try{const t=localStorage.getItem(this.STORAGE_PREFIX+e);if(!t)return null;const{data:a,timestamp:n}=JSON.parse(t),i=Date.now()-n;return i>this.CACHE_EXPIRY_MS?(localStorage.removeItem(this.STORAGE_PREFIX+e),console.log(`[BookingData] Storage expired for ${e}`),null):(console.log(`[BookingData] Storage hit for ${e} (age: ${Math.round(i/6e4)}min)`),a)}catch(t){return console.warn("[BookingData] Storage read error:",t),null}}saveToStorage(e,t){try{const a=JSON.stringify({data:t,timestamp:Date.now()});localStorage.setItem(this.STORAGE_PREFIX+e,a),console.log(`[BookingData] Saved to storage: ${e}`)}catch(a){console.warn("[BookingData] Storage write error:",a)}}async loadData(e,t,a=null){const n=this.getCacheKey(e,t,a);if(this.dataCache.has(n))return console.log(`[BookingData] Memory cache hit for ${n}`),this.dataCache.get(n);const i=this.getFromStorage(n);if(i)return this.dataCache.set(n,i),i;if(this.loadingPromises.has(n))return console.log(`[BookingData] Waiting for existing load: ${n}`),this.loadingPromises.get(n);const s=this._fetchData(e,t,a);this.loadingPromises.set(n,s);try{const o=await s;return this.dataCache.set(n,o),this.saveToStorage(n,o),console.log(`[BookingData] Loaded ${o.length} items for ${n}`),o}finally{this.loadingPromises.delete(n)}}async _fetchData(e,t,a){const n=B[e];if(!n)throw new Error(`Unknown page type: ${e}`);const i=n.file(t,a),s=n.variableName(t,a),o=`${G}/${i}`;return console.log(`[BookingData] Loading script: ${o}`),console.log(`[BookingData] Expected variable: ${s}`),window[s]&&Array.isArray(window[s])?(console.log("[BookingData] Variable already exists, using cached data"),window[s]):document.querySelector(`script[src="${o}"]`)&&(await this._waitForVariable(s,2e3),window[s])?window[s]:new Promise((l,u)=>{const g=document.createElement("script");g.src=o,g.async=!0,g.className="booking-data-script",g.onload=async()=>{console.log(`[BookingData] Script loaded: ${i}`);try{await this._waitForVariable(s,3e3),window[s]&&Array.isArray(window[s])?(console.log(`[BookingData] Found ${window[s].length} items`),l(window[s])):u(new Error(`Variable ${s} not found after script load`))}catch(m){u(m)}},g.onerror=()=>{console.error(`[BookingData] Failed to load script: ${o}`),u(new Error(`Failed to load ${i}`))},document.body.appendChild(g)})}_waitForVariable(e,t=3e3){return new Promise((a,n)=>{const i=Date.now(),s=()=>{window[e]!==void 0?a():Date.now()-i>t?n(new Error(`Timeout waiting for ${e}`)):setTimeout(s,50)};s()})}normalizeText(e){return e.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase()}search(e,t,a=50){if(!e||e.length<3)return[];const n=this.normalizeText(e),i=[];for(const s of t){if(i.length>=a)break;this.normalizeText(s[0]||"").includes(n)&&i.push(s)}return i}formatResult(e,t){const a=e[e.length-1];switch(t){case"hotelPage":return{name:e[0],subtitle:e[1]||"",url:a};case"airportPage":return{name:e[0],subtitle:e[1]||"",url:a};case"districtPage":return{name:e[0],subtitle:e[1]||"",url:a};default:return{name:e[0],subtitle:e.length>2?e[1]:"",url:a}}}hasSubType(e){return B[e]?.hasSubType||!1}clearCache(){this.dataCache.clear(),console.log("[BookingData] Cache cleared")}}const k=new O;class F{constructor(){this.currentPage="booking",this.bookingState={data:null,isLoading:!1,error:null,searchTimeout:null}}async init(){await c.init(),console.log(`[App] Language detected: ${c.getLanguage()}`),this.setupNavigation(),this.setupLanguageSelector(),this.loadPage(this.currentPage)}setupNavigation(){document.querySelectorAll("[data-page]").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault(),this.loadPage(t.currentTarget.dataset.page)})})}setupLanguageSelector(){const e=document.querySelector(".language-dropdown"),t=document.getElementById("langTrigger");document.getElementById("langMenu");const a=document.querySelectorAll(".lang-option"),n={it:{flag:"🇮🇹",code:"IT"},en:{flag:"🇬🇧",code:"EN"},es:{flag:"🇪🇸",code:"ES"},fr:{flag:"🇫🇷",code:"FR"},de:{flag:"🇩🇪",code:"DE"}},i=c.getLanguage();this.updateLangTrigger(t,n[i]),document.querySelector(`[data-lang="${i}"]`)?.classList.add("active"),t?.addEventListener("click",s=>{s.stopPropagation(),e.classList.toggle("is-open"),t.setAttribute("aria-expanded",e.classList.contains("is-open"))}),document.addEventListener("click",s=>{e?.contains(s.target)||(e?.classList.remove("is-open"),t?.setAttribute("aria-expanded","false"))}),a.forEach(s=>{s.addEventListener("click",async()=>{const o=s.dataset.lang;a.forEach(d=>d.classList.remove("active")),s.classList.add("active"),this.updateLangTrigger(t,n[o]),e.classList.remove("is-open"),t.setAttribute("aria-expanded","false"),await c.setLanguage(o),k.clearCache(),this.bookingState.data=null,this.loadPage(this.currentPage),p.success(c.t("languageChanged"))})})}updateLangTrigger(e,t){!e||!t||(e.querySelector(".lang-flag").textContent=t.flag,e.querySelector(".lang-code").textContent=t.code)}loadPage(e){this.currentPage=e;const t=document.getElementById("page-content");document.querySelectorAll("[data-page]").forEach(n=>{n.classList.toggle("active",n.dataset.page===e)}),({booking:()=>{t.innerHTML=this.renderBookingPage(),this.initBookingPage()},tradedoubler:()=>{t.innerHTML=this.renderTradedoublerPage(),this.initTradedoublerPage()},getyourguide:()=>{t.innerHTML=this.renderGetYourGuidePage(),this.initGetYourGuidePage()},civitatis:()=>{t.innerHTML=this.renderCivitatisPage(),this.initCivitatisPage()},carrental:()=>{t.innerHTML=this.renderCarRentalPage(),this.initCarRentalPage()},imgtool:()=>{t.innerHTML=this.renderImageToolPage(),this.initImageToolPage()},feedback:()=>{t.innerHTML=this.renderFeedbackPage(),this.initFeedbackPage()}}[e]||(()=>{t.innerHTML="<p>Page not found</p>"}))(),c.translatePage()}renderBookingPage(){return`
    ...
    `).join(""),e.querySelectorAll("[data-copy]").forEach(a=>{a.addEventListener("click",async()=>{const n=await h.copyToClipboard(a.dataset.copy);p[n?"success":"error"](c.t(n?"deepLinkCopied":"copyError"))})})}renderTradedoublerPage(){return`
      <div class="page-header">
      ...
      <div class="card td-unified-card">
        ...
        <div class="td-site-selector">
          <select id="islandSelect" class="form-select">
            ...
            ${h.getAllIslands().map(t=>`<option value="${t.code}">${t.displayName}</option>`).join("")}
          </select>
        </div>
    ...
    `}initTradedoublerPage(){const e=document.getElementById("islandSelect"),t=document.getElementById("generateBtn"),a=document.getElementById("inputUrl"),n=document.getElementById("resultContainer"),i=document.getElementById("resultLink"),s=document.getElementById("programCardsContainer"),o=document.getElementById("partnerFeedback");e&&new f(e);const d=u=>{const g=$[u]||[];if(!u||g.length===0){s.innerHTML=`
          <div class="program-cards-empty">
            <span class="empty-icon">📋</span>
            <p data-i18n="selectSiteToViewPrograms">Seleziona un sito per vedere i programmi attivi</p>
          </div>
        `;return}const m=g.map(y=>{const v=h.getPartnerBaseLink(u,y),L=I[y]||"—",E=T[y]||"ALTRO";return`
          <div class="program-card">
            <div class="program-card-body">
              <div class="program-card-meta">
                <span class="program-badge program-badge--${E.toLowerCase().replace(/\s+/g,"-")}">${E}</span>
                <span class="program-id">ID: ${L}</span>
              </div>
              <h4 class="program-card-title">${this.escapeHtml(y)}</h4>
              <p class="program-card-link">${v?new URL(v).hostname:"tradedoubler.com"}</p>
            </div>
            <div class="program-card-actions">
              <button class="btn btn-sm btn-primary" data-copy="${this.escapeHtml(v||"")}" title="${c.t("copyLink")}">
                ${c.t("copyLink")}
              </button>
              <a href="${v||"#"}" target="_blank" class="btn btn-sm btn-outline" title="${c.t("openLink")}">
                ${c.t("openLink")}
              </a>
            </div>
          </div>
        `}).join("");s.innerHTML=`
        <div class="program-cards-count">${g.length} ${c.t("programsActive")||"programmi attivi"}</div>
        <div class="program-cards-wrapper">${m}</div>
      `,s.querySelectorAll("[data-copy]").forEach(y=>{y.addEventListener("click",async()=>{const v=y.dataset.copy;if(v){const L=await h.copyToClipboard(v);p[L?"success":"error"](c.t(L?"deepLinkCopied":"copyError"))}})})},l=()=>{const u=e.value,g=a.value.trim();if(!u||!g){o.style.display="none";return}const m=h.detectPartnerFromUrl(g,u);m?(o.innerHTML=`
          <div class="feedback-success">
            <span class="feedback-icon">✓</span>
            <span class="feedback-text">
              <strong>${m.partner}</strong>
              <span class="feedback-meta">${m.category} | ID: ${m.programId||"—"}</span>
            </span>
          </div>
        `,o.style.display="block"):(o.innerHTML=`
          <div class="feedback-error">
            <span class="feedback-icon">✗</span>
            <span class="feedback-text">${c.t("partnerNotRecognized")||"Programma non riconosciuto per questo sito"}</span>
          </div>
        `,o.style.display="block")};e.addEventListener("change",()=>{const u=e.value;d(u),l(),n.style.display="none"}),a.addEventListener("input",l),t.addEventListener("click",()=>{const u=e.value,g=a.value.trim();if(!u){p.warning(c.t("selectSite")||"Seleziona un sito");return}if(!g){p.warning(c.t("enterUrl"));return}const m=h.generateDeepLink(g,u);m.success?(i.href=m.link,i.textContent=m.link,n.style.display="block",p.success(`${c.t("deepLinkGenerated")} (${m.partner})`)):p.error(c.t(m.error)||m.error)}),document.getElementById("copyResultBtn")?.addEventListener("click",async()=>{const u=await h.copyToClipboard(i.href);p[u?"success":"error"](c.t(u?"deepLinkCopied":"copyError"))}),document.getElementById("openResultBtn")?.addEventListener("click",()=>{h.openLink(i.href)})}renderGetYourGuidePage(){return`
      <div class="page-header">
    ...
    `}initGetYourGuidePage(){const e={it:"https://www.getyourguide.it/?partner_id=Q5TFESQ&utm_medium=online_publisher",es:"https://www.getyourguide.es/?partner_id=Q5TFESQ&utm_medium=online_publisher",en:"https://www.getyourguide.com/?partner_id=Q5TFESQ&utm_medium=online_publisher",fr:"https://www.getyourguide.fr/?partner_id=Q5TFESQ&utm_medium=online_publisher"},t=document.getElementById("gygLangSelect"),a=document.getElementById("gygHomepageLink");t&&new f(t);const n=()=>{const l=e[t.value]||e.en;a.href=l,a.textContent=l};t.addEventListener("change",n),n(),document.getElementById("gygHomepageCopyBtn")?.addEventListener("click",async()=>{const l=await h.copyToClipboard(a.href);p[l?"success":"error"](c.t(l?"deepLinkCopied":"copyError"))}),document.getElementById("gygHomepageOpenBtn")?.addEventListener("click",()=>{h.openLink(a.href)});const i=document.getElementById("gygInput"),s=document.getElementById("gygGenerateBtn"),o=document.getElementById("gygResult"),d=document.getElementById("gygResultLink");s.addEventListener("click",()=>{const l=i.value.trim();if(!l){p.warning(c.t("validGetYourGuideURL"));return}const u=h.generateGetYourGuideLink(l);u.success?(d.href=u.link,d.textContent=u.link,o.style.display="block",p.success(c.t("deepLinkGenerated"))):p.error(c.t(u.error))}),document.getElementById("gygCopyBtn")?.addEventListener("click",async()=>{const l=await h.copyToClipboard(d.href);p[l?"success":"error"](c.t(l?"deepLinkCopied":"copyError"))}),document.getElementById("gygOpenBtn")?.addEventListener("click",()=>{h.openLink(d.href)})}renderCivitatisPage(){return`
      <div class="page-header">
    ...
    `}initCivitatisPage(){const e={it:"https://www.civitatis.com/it/?aid=5488",es:"https://www.civitatis.com/es/?aid=5488",en:"https://www.civitatis.com/en/?aid=5488",fr:"https://www.civitatis.com/fr/?aid=5488"},t=document.getElementById("civLangSelect"),a=document.getElementById("civHomepageLink");t&&new f(t);const n=()=>{const l=e[t.value]||e.en;a.href=l,a.textContent=l};t.addEventListener("change",n),n(),document.getElementById("civHomepageCopyBtn")?.addEventListener("click",async()=>{const l=await h.copyToClipboard(a.href);p[l?"success":"error"](c.t(l?"deepLinkCopied":"copyError"))}),document.getElementById("civHomepageOpenBtn")?.addEventListener("click",()=>{h.openLink(a.href)});const i=document.getElementById("civInput"),s=document.getElementById("civGenerateBtn"),o=document.getElementById("civResult"),d=document.getElementById("civResultLink");s.addEventListener("click",()=>{const l=i.value.trim();if(!l){p.warning(c.t("validCivitatisURL"));return}const u=h.generateCivitatisLink(l);u.success?(d.href=u.link,d.textContent=u.link,o.style.display="block",p.success(c.t("deepLinkGenerated"))):p.error(c.t(u.error))}),document.getElementById("civCopyBtn")?.addEventListener("click",async()=>{const l=await h.copyToClipboard(d.href);p[l?"success":"error"](c.t(l?"deepLinkCopied":"copyError"))}),document.getElementById("civOpenBtn")?.addEventListener("click",()=>{h.openLink(d.href)})}renderCarRentalPage(){return`
      <div class="page-header">
      ...
      <div class="card">
        <div class="form-group">
          ...
          <select id="carProviderSelect" class="form-select">
            ${P.map((e,t)=>`<option value="${t}">${e.name}</option>`).join("")}
          </select>
        </div>
    ...
    `}initCarRentalPage(){const e=document.getElementById("carProviderSelect"),t=document.getElementById("carLangSelect"),a=document.getElementById("carResultLink");e&&new f(e),t&&new f(t);const n=()=>{const i=P[e.value],s=t.value,o=i.urls[s]||i.urls.en||Object.values(i.urls)[0];a.href=o,a.textContent=o};e.addEventListener("change",n),t.addEventListener("change",n),n(),document.getElementById("carCopyBtn")?.addEventListener("click",async()=>{const i=await h.copyToClipboard(a.href);p[i?"success":"error"](c.t(i?"deepLinkCopied":"copyError"))}),document.getElementById("carOpenBtn")?.addEventListener("click",()=>{h.openLink(a.href)})}renderImageToolPage(){return`
      <div class="page-header">
    ...
    `}initImageToolPage(){const e=document.getElementById("dropZone"),t=document.getElementById("imageInput"),a=document.getElementById("qualitySlider"),n=document.getElementById("qualityValue"),i=document.getElementById("formatSelect"),s=document.getElementById("imagesContainer"),o=document.getElementById("downloadAllContainer");i&&new f(i),this.compressedImages=[],a.addEventListener("input",()=>{n.textContent=`${a.value}%`}),e.addEventListener("click",()=>t.click()),["dragenter","dragover","dragleave","drop"].forEach(d=>{e.addEventListener(d,l=>{l.preventDefault(),l.stopPropagation()})}),e.addEventListener("dragenter",()=>e.classList.add("drag-over")),e.addEventListener("dragover",()=>e.classList.add("drag-over")),e.addEventListener("dragleave",()=>e.classList.remove("drag-over")),e.addEventListener("drop",d=>{e.classList.remove("drag-over");const l=d.dataTransfer.files;this.processImages(l)}),t.addEventListener("change",()=>{this.processImages(t.files)}),document.getElementById("clearAllBtn")?.addEventListener("click",()=>{this.compressedImages=[],s.innerHTML="",o.style.display="none"}),document.getElementById("downloadAllBtn")?.addEventListener("click",async()=>{this.compressedImages.length!==0&&await this.downloadAllAsZip()})}async processImages(e){const t=document.getElementById("qualitySlider").value/100,a=document.getElementById("formatSelect").value,n=document.getElementById("imagesContainer"),i=document.getElementById("downloadAllContainer");for(const s of e)if(s.type.startsWith("image/"))try{const o=await this.compressImage(s,t,a);this.compressedImages.push(o),this.renderImageCard(o,n),this.updateTotalStats(),i.style.display="block"}catch(o){console.error("Compression error:",o),p.error(`Errore: ${s.name}`)}}compressImage(e,t,a){return new Promise((n,i)=>{const s=new FileReader;s.onload=o=>{const d=new Image;d.onload=()=>{const l=document.createElement("canvas");l.width=d.naturalWidth,l.height=d.naturalHeight,l.getContext("2d").drawImage(d,0,0),l.toBlob(g=>{if(!g){i(new Error("Compression failed"));return}const m=a==="image/webp"?"webp":a==="image/png"?"png":"jpg",y=e.name.replace(/\.[^/.]+$/,"");n({originalName:e.name,compressedName:`${y}_compressed.${m}`,originalSize:e.size,compressedSize:g.size,reduction:((1-g.size/e.size)*100).toFixed(1),blob:g,url:URL.createObjectURL(g)})},a,t)},d.onerror=i,d.src=o.target.result},s.onerror=i,s.readAsDataURL(e)})}renderImageCard(e,t){const a=document.createElement("div");a.className="image-card";const n=s=>s>1024*1024?`${(s/1024/1024).toFixed(2)} MB`:`${(s/1024).toFixed(1)} KB`,i=parseFloat(e.reduction)>50?"text-success":parseFloat(e.reduction)>20?"text-warning":"text-muted";a.innerHTML=`
    ...
    `,a.querySelector(".download-btn").addEventListener("click",()=>{const s=document.createElement("a");s.href=e.url,s.download=e.compressedName,s.click()}),a.querySelector(".remove-btn").addEventListener("click",()=>{const s=this.compressedImages.findIndex(o=>o.url===e.url);s>-1&&(URL.revokeObjectURL(e.url),this.compressedImages.splice(s,1),a.remove(),this.updateTotalStats(),this.compressedImages.length===0&&(document.getElementById("downloadAllContainer").style.display="none"))}),t.appendChild(a)}updateTotalStats(){const e=document.getElementById("totalStats");if(!e||this.compressedImages.length===0)return;const t=this.compressedImages.reduce((s,o)=>s+o.originalSize,0),a=this.compressedImages.reduce((s,o)=>s+o.compressedSize,0),n=((1-a/t)*100).toFixed(1),i=s=>s>1024*1024?`${(s/1024/1024).toFixed(2)} MB`:`${(s/1024).toFixed(1)} KB`;e.textContent=`${this.compressedImages.length} ${c.t("images")} • ${i(t)} → ${i(a)} (-${n}%)`}async downloadAllAsZip(){if(!window.JSZip){const n=document.createElement("script");n.src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js",document.head.appendChild(n),await new Promise(i=>n.onload=i)}const e=new JSZip;for(const n of this.compressedImages)e.file(n.compressedName,n.blob);const t=await e.generateAsync({type:"blob"}),a=document.createElement("a");a.href=URL.createObjectURL(t),a.download="compressed_images.zip",a.click(),URL.revokeObjectURL(a.href),p.success(c.t("downloadStarted"))}renderFeedbackPage(){return`
      <div class="page-header">
